...
const AllOrders = () => {
    ...
    return (
        <div className='py-12 overflow-x-auto  px-3 flex-1' >
            <h3 className="uppercase text-center py-5 text-xl md:text-2xl font-semibold text-neutral-800/90">All Orders</h3>
            <div className="h-full w-full overflow-auto ">
                <table className="w-full text-xs  sm:text-sm lg:text-base  min-h-[30vh] min-w-[453px] h-full  ">
                    <thead>
                            ...
                            allOrders?.map(order => {
                                return (
                                    <tr key={order._id} className="grid grid-cols-12 gap-3 even:bg-gray-500/10 odd:bg-transparent p-1">
                                        ...
                                        <td className="col-span-3 break-words text-left ps-3" > <Link to={`/orderdetails/${order._id}`}>{order.orderStatus} </Link></td>
